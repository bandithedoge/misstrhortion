...
MisstortionAudioProcessorEditor::MisstortionAudioProcessorEditor(
    ...
    m_backgroundImage = ImageCache::getFromMemory(
        BinaryData::Misstortion_png, BinaryData::Misstortion_pngSize);

    setSize(m_backgroundImage.getWidth() * 2,
            m_backgroundImage.getHeight() * 2);

    InitializeSlider(m_sliderMix, Slider::LinearBar, processor.m_paramMix, "%");
    InitializeSlider(m_sliderGainIn, Slider::LinearBar, processor.m_paramGainIn,
                     " dB");
    InitializeSlider(m_sliderGainOut, Slider::LinearBar,
                     processor.m_paramGainOut, " dB");

    InitializeSlider(m_sliderDriveHard, Slider::LinearBar,
                     processor.m_paramDriveHard, " dB");
    InitializeSlider(m_sliderDriveSoft, Slider::LinearBar,
                     processor.m_paramDriveSoft, " dB");
    InitializeSlider(m_sliderToneHP, Slider::LinearBar, processor.m_paramToneHP,
                     0.25, " Hz");
    InitializeSlider(m_sliderToneLP, Slider::LinearBar, processor.m_paramToneLP,
                     0.25, " Hz");
    InitializeSlider(m_sliderSymmetry, Slider::LinearBar,
                     processor.m_paramSymmetry, "%");
...
void MisstortionAudioProcessorEditor::InitializeSlider(
    Slider &slider, Slider::SliderStyle style, AudioParameterFloat *param,
    String suffix) {
    ...
    if (baseParam != nullptr) {
        double defaultValue =
            (double)range.convertFrom0to1(baseParam->getDefaultValue());
...
void MisstortionAudioProcessorEditor::InitializeSlider(
    Slider &slider, Slider::SliderStyle style, AudioParameterInt *param,
    double skew, String suffix) {
    ...
    if (baseParam != nullptr) {
        double defaultValue =
            Lerp((double)range.getStart(), (double)range.getEnd(),
                 baseParam->getDefaultValue());
...
//==============================================================================
void MisstortionAudioProcessorEditor::paintFilterButton(ToggleButton &button,
                                                        Graphics &g) {
...
void MisstortionAudioProcessorEditor::paint(Graphics &g) {
    ...
    g.drawImage(m_backgroundImage, 0, 0, width * 2, height * 2, 0, 0, width,
                height);
}
...
void MisstortionAudioProcessorEditor::setControlBounds(Component &component,
                                                       int x, int y, int w,
                                                       int h) {
    component.setBounds(x * 2, y * 2, w * 2, h * 2);
}

void MisstortionAudioProcessorEditor::setSliderParam(
    Slider *sliderChanged, Slider &actualSlider, AudioParameterFloat *param) {
    if (sliderChanged == &actualSlider) {
        *param = (float)sliderChanged->getValue();
    }
}

void MisstortionAudioProcessorEditor::setSliderParam(Slider *sliderChanged,
                                                     Slider &actualSlider,
...
void MisstortionAudioProcessorEditor::timerCallback() {
    ...
    m_sliderDriveHard.setValue(*processor.m_paramDriveHard,
                               dontSendNotification);
    m_sliderDriveSoft.setValue(*processor.m_paramDriveSoft,
                               dontSendNotification);
    ...
    m_buttonFilterModeLegacy.setToggleState(filterMode == 0,
                                            dontSendNotification);
    m_buttonFilterMode6db.setToggleState(filterMode == 1, dontSendNotification);
    m_buttonFilterMode12db.setToggleState(filterMode == 2,
                                          dontSendNotification);

#if DEBUG
    m_labelDebug.setText("[DEBUG] " + processor.m_debugText,
                         dontSendNotification);
#endif
}
